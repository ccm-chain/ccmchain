...
func newTestClientHandler(backend *backends.SimulatedBackend, odr *LesOdr, indexers []*core.ChainIndexer, db database.Database, peers *peerSet, ulcServers []string, ulcFraction int) *clientHandler {
	var (
...
func newTestServerHandler(blocks int, indexers []*core.ChainIndexer, db database.Database, peers *peerSet, clock mclock.Clock) (*serverHandler, *backends.SimulatedBackend) {
	var (
		gspec = core.Genesis{
			Config:   params.AllEthashProtocolChanges,
			Alloc:    core.GenesisAlloc{bankAddr: {Balance: bankFunds}},
