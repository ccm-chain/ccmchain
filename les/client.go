...
func New(stack *node.Node, config *protocol.Config) (*LightEthereum, error) {
	chainDb, err := stack.OpenDatabase("lightchaindata", config.DatabaseCache, config.DatabaseHandles, "ccm/db/chaindata/")
	if err != nil {
		return nil, err
	}
	lespayDb, err := stack.OpenDatabase("lespay", 0, 0, "ccm/db/lespay")
	if err != nil {
		return nil, err
	}
	...
	leth := &LightEthereum{
		...
		engine:         protocol.CreateConsensusEngine(stack, chainConfig, &config.Ethash, nil, false, chainDb),
		bloomRequests:  make(chan chan *bloombits.Retrieval),
		bloomIndexer:   protocol.NewBloomIndexer(chainDb, params.BloomBitsBlocksClient, params.HelperTrieConfirmations),
...
func (s *LightEthereum) Stop() error {
	...
	s.chainDb.Close()
	s.wg.Wait()
	log.Info("Light ccmchain stopped")
	return nil
}
