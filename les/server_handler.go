...
func newServerHandler(server *LesServer, blockchain *core.BlockChain, chainDb database.Database, txpool *core.TxPool, synced func() bool) *serverHandler {
...
func (h *serverHandler) handle(p *clientPeer) error {
	...
	if cap, err := h.server.clientPool.connect(p); cap != p.fcParams.MinRecharge || err != nil {
		p.Log().Debug("Light Ccmchain peer rejected", "err", errFullClientPool)
