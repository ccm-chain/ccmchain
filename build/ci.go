...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = ioutil.TempDir("", "gccm-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doAndroidArchive(cmdline []string) {
	...
	build.MustRun(gomobileTool("bind", "-ldflags", "-s -w", "--target", "android", "--javapkg", "org.ethereum", "-v", "github.com/ccm-chain/ccmchain/mobile"))

	if *local {
		// If we're building locally, copy bundle to build dir and skip Maven
		os.Rename("gccm.aar", filepath.Join(GOBIN, "gccm.aar"))
		os.Rename("gccm-sources.jar", filepath.Join(GOBIN, "gccm-sources.jar"))
		return
	}
	...
	archive := "gccm-" + archiveBasename("android", params.ArchiveVersion(env.Commit)) + ".aar"
	os.Rename("gccm.aar", archive)
...
func doXCodeFramework(cmdline []string) {
	...
	bind := gomobileTool("bind", "-ldflags", "-s -w", "--target", "ios", "-v", "github.com/ccm-chain/ccmchain/mobile")
