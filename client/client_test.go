...
var (
	_ = ccmchain.ChainReader(&Client{})
	_ = ccmchain.TransactionReader(&Client{})
	_ = ccmchain.ChainStateReader(&Client{})
	_ = ccmchain.ChainSyncReader(&Client{})
	_ = ccmchain.ContractCaller(&Client{})
	_ = ccmchain.GasEstimator(&Client{})
	_ = ccmchain.GasPricer(&Client{})
	_ = ccmchain.LogFilterer(&Client{})
	_ = ccmchain.PendingStateReader(&Client{})
	// _ = ccmchain.PendingStateEventer(&Client{})
	_ = ccmchain.PendingContractCaller(&Client{})
)
...
func TestToFilterArg(t *testing.T) {
	...
	for _, testCase := range []struct {
		name   string
		input  ccmchain.FilterQuery
		output interface{}
		err    error
	}{
		{
			"without BlockHash",
			ccmchain.FilterQuery{
				Addresses: addresses,
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	ethservice, err := protocol.New(n, config)
	if err != nil {
		t.Fatalf("can't create new ccmchain service: %v", err)
	}
