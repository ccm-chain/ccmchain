...
func InitDatabaseFromFreezer(db database.Database) {
...
func iterateTransactions(db database.Database, from uint64, to uint64, reverse bool) (chan *blockTxHashes, chan struct{}) {
...
func IndexTransactions(db database.Database, from uint64, to uint64) {
...
func UnindexTransactions(db database.Database, from uint64, to uint64) {
	// short circuit for invalid range
	if from >= to {
		return
	}
