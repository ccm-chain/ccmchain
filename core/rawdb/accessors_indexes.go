...
func writeTxLookupEntry(db database.KeyValueWriter, hash common.Hash, numberBytes []byte) {
...
// a list of hashes
func WriteTxLookupEntries(db database.KeyValueWriter, number uint64, hashes []common.Hash) {
...
func WriteTxLookupEntriesByBlock(db database.KeyValueWriter, block *types.Block) {
...
func WriteTxLookupEntriesByHash(db database.KeyValueWriter, number uint64, hashes []common.Hash) {
	numberBytes := new(big.Int).SetUint64(number).Bytes()
	for _, hash := range hashes {
		if err := db.Put(txLookupKey(hash), numberBytes); err != nil {
			log.Crit("Failed to store transaction lookup entry", "err", err)
...
func DeleteTxLookupEntries(db database.KeyValueWriter, hashes []common.Hash) {
	for _, hash := range hashes {
...
func ReadTransaction(db database.Reader, hash common.Hash) (*types.Transaction, common.Hash, uint64, uint64) {
...
// its added positional metadata.
func ReadReceipt(db database.Reader, hash common.Hash, config *params.ChainConfig) (*types.Receipt, common.Hash, uint64, uint64) {
...
func DeleteBloombits(db database.Database, bit uint, from uint64, to uint64) {
