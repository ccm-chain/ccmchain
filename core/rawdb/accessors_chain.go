...
func ReadFastTrieProgress(db database.KeyValueReader) uint64 {
...
func ReadHeader(db database.Reader, hash common.Hash, number uint64) *types.Header {
...
// to-number mapping.
func WriteHeader(db database.KeyValueWriter, header *types.Header) {
	var (
		hash   = header.Hash()
...
func WriteBodyRLP(db database.KeyValueWriter, hash common.Hash, number uint64, rlp rlp.RawValue) {
...
func ReadBody(db database.Reader, hash common.Hash, number uint64) *types.Body {
...
func WriteBody(db database.KeyValueWriter, hash common.Hash, number uint64, body *types.Body) {
...
func ReadRawReceipts(db database.Reader, hash common.Hash, number uint64) types.Receipts {
...
func ReadReceipts(db database.Reader, hash common.Hash, number uint64, config *params.ChainConfig) types.Receipts {
...
func WriteReceipts(db database.KeyValueWriter, hash common.Hash, number uint64, receipts types.Receipts) {
...
func ReadBlock(db database.Reader, hash common.Hash, number uint64) *types.Block {
...
func WriteBlock(db database.KeyValueWriter, block *types.Block) {
	WriteBody(db, block.Hash(), block.NumberU64(), block.Body())
	WriteHeader(db, block.Header())
}

// WriteAncientBlock writes entire block data into ancient store and returns the total written size.
func WriteAncientBlock(db database.AncientWriter, block *types.Block, receipts types.Receipts, td *big.Int) int {
...
func FindCommonAncestor(db database.Reader, a, b *types.Header) *types.Header {
