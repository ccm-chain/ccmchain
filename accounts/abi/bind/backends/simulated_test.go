...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != ccmchain.NotFound {
		t.Fatalf("err should be `ccmchain.NotFound` but received %v", err)
	}
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != params.AllProtocolChanges {
		t.Errorf("expected sim config to equal params.AllProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AllProtocolChanges {
		t.Errorf("expected sim blockchain config to equal params.AllProtocolChanges, got %v", sim.config)
	}
...
func TestSimulatedBackend_EstimateGas(t *testing.T) {
	...
	var cases = []struct {
		name        string
		message     ccmchain.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", ccmchain.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", ccmchain.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestSimulatedBackend_EstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", ccmchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", ccmchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", ccmchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", ccmchain.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
...
func TestSimulatedBackend_PendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, ccmchain.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, ccmchain.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestSimulatedBackend_CallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, ccmchain.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, ccmchain.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
