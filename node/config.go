...
import (
	...
	"github.com/ccm-chain/ccmchain/accounts/scwallet"
	"github.com/ccm-chain/ccmchain/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gccm is "gccm". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Gccm", keep that.
	if name == "gccm" || name == "gccm-testnet" {
		name = "Gccm"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gccm" instances.
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gccm 1.4 are used if they exist.
	if warn, isOld := isOldGethResource[path]; isOld {
		...
		if oldpath != "" && common.FileExist(oldpath) {
			if warn {
				c.warnOnce(&c.oldGethResourceWarning, "Using deprecated resource file %s, please move this file to the 'gccm' subdirectory of datadir.", oldpath)
			}
			return oldpath
		}
	}
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "ccmchain-keystore")
		ephemeral = keydir
	}
