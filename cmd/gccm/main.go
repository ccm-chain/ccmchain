...
const (
	clientIdentifier = "gccm" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the ccmchain command line interface")
...
func prepare(ctx *cli.Context) {
	...
	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting Gccm on Ccmchain mainnet...")
	}
...
// miner.
func startNode(ctx *cli.Context, stack *node.Node, backend api.Backend) {
	...
	// Create a client to interact with local gccm node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		ethBackend, ok := backend.(*protocol.EthAPIBackend)
		if !ok {
