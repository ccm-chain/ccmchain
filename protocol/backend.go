...
func New(stack *node.Node, config *Config) (*Ethereum, error) {
	...
	if config.SyncMode == downloader.LightSync {
		return nil, errors.New("can't run ccm.Ccmchain in light sync mode, use les.LightEthereum")
	}
	...
	chainDb, err := stack.OpenDatabaseWithFreezer("chaindata", config.DatabaseCache, config.DatabaseHandles, config.DatabaseFreezer, "ccm/db/chaindata/")
	if err != nil {
		return nil, err
	}
	...
	if !config.SkipBcVersionCheck {
		if bcVersion != nil && *bcVersion > core.BlockChainVersion {
			return nil, fmt.Errorf("database version is v%d, Gccm %s only supports v%d", *bcVersion, params.VersionWithMeta, core.BlockChainVersion)
	...
	*/
	if eth.protocolManager, err = NewProtocolManager(chainConfig, nil, config.SyncMode, config.NetworkId, eth.eventMux, eth.txPool, eth.engine, eth.blockchain, chainDb, cacheLimit, config.Whitelist); err != nil {
		return nil, err
	}
	...
	eth.netRPCService = api.NewPublicNetAPI(eth.p2pServer, eth.NetVersion())
