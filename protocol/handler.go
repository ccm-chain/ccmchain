...
import (
	...
	"github.com/ccm-chain/ccmchain/core/forkid"
	...
	"github.com/ccm-chain/ccmchain/protocol/fetcher"
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing Ccmchain peer", "peer", id)
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping Ccmchain protocol")
	...
	log.Info("Ccmchain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Ccmchain peer connected", "name", p.Name())
	...
	if err := p.Handshake(pm.networkID, td, hash, genesis.Hash(), forkid.NewID(pm.blockchain), pm.forkFilter); err != nil {
		p.Log().Debug("Ccmchain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Ccmchain peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Ccmchain message handling failed", "err", err)
			return err
		}
	}
}
